...
def min_cost_to_connect_cables(cable_lengths):
    ...
    while len(cable_lengths) > 1:
        ...
        print("after first pop of minimal element", cable_lengths)

        second=heapq.heappop(cable_lengths)
        print("after second pop of minimal element", cable_lengths)
        # Об'єднуємо кабелі та додаємо вартість об'єднання до загальних витрат
        cost= first+second
        print(f"Merged cabels with {first} and {second} lengths with cost {cost}")       
        total_cost+=cost
         # Додаємо новий об'єднаний кабель до купи
        heapq.heappush(cable_lengths, cost)
        print("After push: ", cable_lengths)
